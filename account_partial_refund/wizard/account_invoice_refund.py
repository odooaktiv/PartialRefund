...
class InvoiceRefundLine(models.TransientModel):
    ...
    invoice_refund_id = fields.Many2one('account.invoice.refund','Invoice Refund')
    ...
    def check_invoice_refund_qty(self):
        if self.refund_qty > self.invoiced_qty:
            raise ValidationError("Refunded Quantity should be less than Invoice Quantity")
...
class AccountInvoiceRefund(models.TransientModel):
    ...
    refund_line_ids = fields.One2many('invoice.refund.line', 'invoice_refund_id', 'Refund lines')
        ...
        if self._context.get('active_id',False):
            ...
            for line in invoice_id.invoice_line_ids:
                if line.to_refund_qty > 0 :
                    refund_lines.append((0,0, {'invoice_line_id': line.id,
